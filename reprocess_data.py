...
def center_image(image):
    """
    Centers and scales the digit within a 28x28 canvas.
    Preserves gradients by using a high-quality resampling filter.
    """
    ...
    # Resize the digit using a high-quality resampling filter to preserve gradients
    resized_digit = cropped.resize((new_width, new_height), resample=Image.BILINEAR)
    # Alternatively, use Image.LANCZOS for even higher quality:
    # resized_digit = cropped.resize((new_width, new_height), resample=Image.LANCZOS)

    # Center the resized digit on a 28x28 canvas
    new_image = Image.new('L', (28, 28), color=0)  # Black background
